...
public class Player {
	...
	public void playTurn() {
		...
		while(this.buys > 0 && this.money > 0){
			cardName = this.chooseCard("Choisis une carte a achet� : ", this.getGame().availableSupplyCards(), true);
			if(!cardName.equals("")) {
				
				Card testBuyCard = new Copper();
				testBuyCard = this.buyCard(cardName);
				if(testBuyCard == null) {
					System.out.println("L'achat a �chou�, v�rifier que la carte existe\nEt que vous avez assez d'argent.");
				}else {
					System.out.println("Vous avez achet� "+ cardName);
				}
